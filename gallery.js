  ...
  if (featuredContainer && typeof currentTreatID !== "undefined") {
    ...
    if (featuredTreat) {
      featuredContainer.innerHTML =
        ...
        currentTreatID +
        '" class="image-link">' +
        '<div class="placeholder-img"><span>' +
        featuredTreat.name +
        "</span></div>" +
        "</a>" +
        "</div>" +
  ...
  if (galleryContainer) {
    function renderGallery(filterText = "") {
      ...
      Object.keys(treatData).forEach(function (id) {
        ...
        if (
          treat.name.toLowerCase().includes(cleanFilter) ||
          ...
          const cardHTML =
            ...
            id +
            '" class="image-link">' +
            // We replaced the div with a real <img> tag here:
            '<img src="' +
            mainImage +
            '" alt="' +
            treat.name +
      ...
      if (matchCount === 0) {
        galleryContainer.innerHTML =
          '<p style="text-align:center; width:100%;">No treats found matching "' +
          filterText +
          '"</p>';
      }
    }
    ...
    if (searchInput) {
      ...
      console.error(
        "Javascript cannot find the element with id 'treat-search'"
      );
    }
  }
});
